...
def train(params, model_dir):
    ...
    train_metadata = metadata[:int(metadata.shape[0]*(1-eval_fraction))]
    ...
    # Data handler 
    train_dh = DataHandler(train_metadata, image_dims, batch_size)
    eval_dh = DataHandler(eval_metadata, image_dims, batch_size)
    train_batcher = train_dh.supply_batch(greenonly=False)
    eval_batcher = eval_dh.supply_batch(greenonly=False)

    num_batches = train_dh.size//batch_size
                    ...
                    if batch % 10 == 0:
                        ...
                        eval_summary, eval_loss, eval_f1_value = sess.run([summary_op, loss, f1_score], 
                                                                          feed_dict={images: eval_images,
                                                                                     labels: eval_labels})
                        # writer.add_summary(eval_summary, epoch*batch_size+batch)
                        writer.add_summary(train_summary, epoch*num_batches+batch)
                        writer.flush()                                
                        eval_writer.add_summary(eval_summary, epoch*batch_size+batch)
                        eval_writer.flush()  
    ...
    scipy.io.savemat(file_name, {'loss': np.array(train_loss_list),
